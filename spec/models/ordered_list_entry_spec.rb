...
RSpec.describe OrderedListEntry do
  ...
  describe 'all_as_array_nested_by_name' do
    it "calls arrange_as_array with the order [ancestry, list_position, name]" do
      expect(described_class).to receive(:arrange_as_array).with(order: %w(ancestry list_position name))
  ...
  describe 'arrange_as_array(options = {}, nodes_to_arrange_hash = nil)' do
    ...
    it 'if no nodes_to_arrange_hash is given, calls arrange to create the hash of nodes' do
      expect(described_class).to receive(:arrange).and_call_original
      described_class.arrange_as_array
    end
    ...
    it 'adds the entry and its children (arranged as an array) to the array returned' do
      ...
      expect(described_class.arrange_as_array).to eq([two_children_list,
                                                      first_child,
                                                      second_child,
                                                      second_child_child])
    end
