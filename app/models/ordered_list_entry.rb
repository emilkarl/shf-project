...
#
# @desc Responsibility: This is an entry that keeps track of the _order_ of
# its children (if any).  Ordering the children is the main difference
# between this and a 'typical' class that uses has_ancestry (the Ancestry gem,
# which provides nested children).
#
#
...
class OrderedListEntry < ApplicationRecord
  ...
  def self.all_as_array_nested_by_name
    arrange_as_array(order: %w(ancestry list_position name))
  end


  # Arrange the hash of nodes and all children as an Array. Sort based on the options given.
  # If no hash of nodes is given, arrange all OrderedListEntries
  # Recurse depth first through all children.
  #
  ...
  #
  # @return [Array<OrderedListEntry>] - an Array with the given node first, then its children sorted based on options[:order]
  #
